...
JOINT_NAMES = ['back_bkz', 'back_bky', 'back_bkx', 'l_arm_shz', 'l_arm_shx', 'l_arm_ely', 'l_arm_elx', 'l_arm_wry', 'l_arm_wrx', 'l_arm_wry2', 'neck_ry', 'r_arm_shz', 'r_arm_shx', 'r_arm_ely', 'r_arm_elx',
               'r_arm_wry', 'r_arm_wrx', 'r_arm_wry2', 'l_leg_hpz', 'l_leg_hpx', 'l_leg_hpy', 'l_leg_kny', 'l_leg_aky', 'l_leg_akx', 'r_leg_hpz', 'r_leg_hpx', 'r_leg_hpy', 'r_leg_kny', 'r_leg_aky', 'r_leg_akx']
...
def pre_process_ref_joint_pos(ref_joint_pos):
    """
    1. align the coordinate: most bvh file use right-hand and y-up axis, while atlas urdf and pybullet use z-up with right-hand coordinate
    ...
    if bvh_cfg.Y_UP_AXIS and (not bvh_cfg.LEFT_HAND_COORDINATE):
        for i in range(num_pos):
            ...
            curr_pos = curr_pos * atlas_cfg.REF_POS_SCALE
            proc_pos[i] = curr_pos
...
def scale_ref_pos(robot, ref_joint_pos):
    ...
    scaled_hips_to_crotch_pos_delta = ref_hips_to_crotch_pos_delta * \
        atlas_cfg.HIPS_TO_CROTCH_SCALE
    scaled_crotch_to_knee_pos_delta = ref_crotch_to_knee_pos_delta * \
        atlas_cfg.CROTCH_TO_KNEE_SCALE
    scaled_knee_to_foot_pos_delta = ref_knee_to_foot_pos_delta * \
        atlas_cfg.KNEE_TO_FOOT_SCALE
    scaled_foot_to_toe_pos_delta = ref_foot_to_toe_pos_delta * atlas_cfg.FOOT_TO_TOE_SCALE
...
def retarget_pose(robot, ref_joint_pos):
    ...
    target_robot_joint_indices = [
        robot_joint_indices['back_bky'], robot_joint_indices['neck_ry'],
        robot_joint_indices['l_arm_elx'], robot_joint_indices['r_arm_elx'],
        robot_joint_indices['l_arm_wrx'], robot_joint_indices['r_arm_wrx'],
        robot_joint_indices['l_leg_kny'], robot_joint_indices['r_leg_kny'],
        robot_joint_indices['l_leg_aky'], robot_joint_indices['r_leg_aky']
    ]
    ...
    DEFAULT_JOINT_POS = np.array(
        joint_lower_limit) + np.array(joint_upper_limit)
    ...
    target_joint_pose = pybullet.calculateInverseKinematics2(robot,
                                                             ...
                                                             jointDamping=atlas_cfg.JOINT_DAMPING*robot_num_joints,
    ...
    #                                                              robot_joint_indices['LLEG_JOINT2'], robot_joint_indices['RLEG_JOINT2'],
...
def build_world():
    ...
    robot = pybullet.loadURDF(atlas_cfg.ROBOT_URDF_FILENAME, basePosition=np.array(
        [0, 0, 0.9]), baseOrientation=np.array([0, 0, 0, 1]))
...
def main():
    ...
    robot_joint_marker_ids = build_markers(
        num_joints, col=[0., 0., 1., 1.], size=0.02)
    scaled_bvh_joint_pos_marker_ids = build_markers(
        num_bvh_joints, col=[0.9, 0., 0.7, 1.], size=0.05)
    ...
    for i in range(len(bvh_file_list)):
        ...
        scaled_ref_joint_pos_frames = np.zeros_like(ref_joint_pos_frames)
        for frame in range(num_frames):
            scaled_ref_pos = scale_ref_pos(robot,
                                           pre_process_ref_joint_pos(ref_joint_pos_frames[frame]))
            scaled_ref_joint_pos_frames[frame] = scaled_ref_pos
        ...
        while True:
            ...
            set_marker_pos(
                scaled_ref_joint_pos_frames[f_idx], scaled_bvh_joint_pos_marker_ids)
            # set_marker_pos(
