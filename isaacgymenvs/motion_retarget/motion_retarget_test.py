...
def set_joint_posi_origin(joint_global_posi):
    """
    eliminate the offset of motions
    set the motions start at the origin
    """
    pos_data_origin = joint_global_posi.copy()
    if bvh_cfg.Y_UP_AXIS:
        x_offset = joint_global_posi[0, 0, 0]
        z_offset = joint_global_posi[0, 0, 2]
...
def get_joint_global_pos(bvh_file):
    ...
    joint_position = set_joint_posi_origin(joint_position)
    return joint_position, joint_names, frame_interval
...
def retarget_pose(robot, ref_joint_pos, robot_joint_indices):
    ...
    # retarget knee
    ref_knee_position = ref_joint_pos[[bvh_cfg.BVH_JOINT_NAMES.index(
    ...
    ref_knee_delta = ref_knee_position - ref_upper_leg_position

    leg_scale = khr_cfg.KHR_LEG_LENGTH / (bvh_cfg.BVH_ROOT_HEIGHT + 0.015)
    # khr_foot_delta = ref_leg_delta * leg_scale
    khr_knee_delta = ref_knee_delta
    khr_lhip_link0_position = np.array(pybullet.getLinkState(
        robot, robot_joint_indices['LLEG_JOINT0'], computeForwardKinematics=True)[4])
    khr_rhip_link0_position = np.array(pybullet.getLinkState(
        robot, robot_joint_indices['RLEG_JOINT0'], computeForwardKinematics=True)[4])
    khr_hip_link0_position = np.vstack(
        (khr_lhip_link0_position, khr_rhip_link0_position))
    target_knee_position = khr_hip_link0_position + khr_knee_delta
    # target_foot_position[:, 2] = ref_foot_position[:, 2] # make the foot clearance more stable

    # TODO: add foot orientation in global
    target_knee_pose = pybullet.calculateInverseKinematics2(robot,
                                                            endEffectorLinkIndices=[
                                                                robot_joint_indices['LLEG_JOINT2'], robot_joint_indices['RLEG_JOINT2']],
                                                            targetPositions=target_knee_position,
                                                            jointDamping=khr_cfg.JOINT_DAMPING*robot_num_joints)

    for k in range(len(non_fixed_joint_indices)):
        pybullet.resetJointState(
            robot, non_fixed_joint_indices[k], target_knee_pose[k], 0.)
    ...
    ref_knee_position = ref_joint_pos[[bvh_cfg.BVH_JOINT_NAMES.index(
        'LeftUpLeg'), bvh_cfg.BVH_JOINT_NAMES.index('RightUpLeg')]]
    ref_foot_delta = ref_foot_position - ref_knee_position

    leg_scale = khr_cfg.KHR_LEG_LENGTH / (bvh_cfg.BVH_ROOT_HEIGHT + 0.015)
    # khr_foot_delta = ref_leg_delta * leg_scale
    khr_foot_delta = ref_foot_delta
    khr_lknee_link0_position = np.array(pybullet.getLinkState(
        robot, robot_joint_indices['LLEG_JOINT2'], computeForwardKinematics=True)[4])
    khr_rknee_link0_position = np.array(pybullet.getLinkState(
        robot, robot_joint_indices['RLEG_JOINT2'], computeForwardKinematics=True)[4])
    khr_knee_link0_position = np.vstack(
        (khr_lknee_link0_position, khr_rknee_link0_position))
    target_foot_position = khr_knee_link0_position + khr_foot_delta
    # target_foot_position[:, 2] = ref_foot_position[:, 2] # make the foot clearance more stable
...
def main():
    ...
    for i in range(len(bvh_file_list)):
        ...
        ref_joint_position, bvh_joint_names, frame_interval = get_joint_global_pos(
            file_path)
        ...
        while True:
            ...
            time_end = time.time()
            sleep_dur = frame_interval - (time_end - time_start)
            sleep_dur = max(0, sleep_dur)

            time.sleep(sleep_dur * 4)
